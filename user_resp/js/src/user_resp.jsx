...
var CusNav = React.createClass({
    render: function() {
        ...
        return <div style={navStyle}>
            ...
            <img src={backImg} style={navBackBtnStyle}/>
        </div>;
    }
});
...
var DeviceInfoBox = React.createClass({
    ...
    render: function() {
        ...
        return <div style={boxStyle}>{info}</div>;
    }
});
...
var CheckBoxCellItem =  React.createClass({
    ...
    render: function() {
        ...
        var checkBoxImg = "";
        if(this.state.inputModel._isSelected){ //选中
            checkBoxImg = "../images/feed_chose_ico@2x.png";
        }else{
            checkBoxImg = "../images/feed_unchose_ico@2x.png";
        }
        return (<div style={cellItemStyle} onClick={this.handleClick}>
            <div style={clearStyleTop}></div>
            <div style={clearStyleRight1}></div>
            <img style={boxBtnStyle} src={checkBoxImg}/>
            <div style={clearStyleRight2}></div>
            <span style={labelStylt}>{this.state.inputModel._name}</span>
        </div>);
    }
});
...
var CheckBoxRange = React.createClass({
    ...
    render: function() {
        ...
        var title = "用户选项";
        return (<div style={boxRangeStyle}>
            <div style={clearStyleTopBottom}></div>
            <div style={clearStyleRight}></div>
            <div style={boxTitleStyle}>{title}</div>
            {
                this.state.inputModel.map(function(modelElement){
                   return  <CheckBoxCellItem inputModel={modelElement}/>;
                })
            }
        </div>);
    }
...
var ResponseContentBox = React.createClass({
    ...
    render: function() {
        ...
        return <textarea  onChange={this.handleChange} style={boxStyle} maxlength='20' placeholder={placeholder} cols={window.screen.availWidth - 10}></textarea>;
    }
});
...
var PhoneNumberBox = React.createClass({
    ...
    render: function() {
        ...
        return (<div style={boxStyle}>
            <div style={topSpaceStyle}></div>
            <div>
                <table style={tableStyle}>
                    <tr>
                        <td>{title}</td>
                        <td style={tdStyle}><input onChange={this.handleChange} ref="phoneInput" type="number" pattern="\d*" name="phone" style={inputBoxStyle}/></td>
                    </tr>
                </table>
            </div>
        </div>);
...
var SubmitBtn = React.createClass({
    ...
    render: function() {
        ...
        return (<div>
            ...
            <input style={submitBtnStyle} type="button" value={btntitle} onClick={this.handleClickSubmit} />
    ...
    render: function() {
        if(this.state.inputModel == null){
            ...
            return (
                <div>
                    <CusNav/>
                    <DeviceInfoBox inputModel={this.state.inputModel._deviceModel}/>
                    <CheckBoxRange inputModel={this.state.inputModel._checkBoxRangeModel._checkBoxModelArray}/>
                    <ResponseContentBox  inputModel={this.state.inputModel._textModel}/>
                    <PhoneNumberBox inputModel={this.state.inputModel._phoneModel}/>
                    <SubmitBtn/>
                </div>
            );
        }
...
var RunApp = function(){
    ...
    ReactDOM.render(
        <CustomBody inputModel={nativeModel}/>,
        document.body
    );
};
RunApp();
